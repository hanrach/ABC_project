...
df <- read_csv(here('data',"covid19-canada.csv")) %>% 
  ...
  mutate(R= numrecover + numdeaths,
         I = numactive) %>% 
  select(date,I,R)

plot(df$I,ylim=c(0,100000))
abline(v=137+32,col='red') # August 10th
abline(v=which(df$date=="2021-01-10"),col='red')

points(df$R,col='orange')
